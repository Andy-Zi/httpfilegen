...
def generate_http_request(path, method, spec_data):
    ...
    full_url = f"https://api.example.com{path}"  # Replace with base URL from spec if available
    ...
    if 'parameters' in spec_data:
        for param in spec_data['parameters']:
            if param['in'] == 'path':
                ...
                full_url = full_url.replace(placeholder, f"{{{param['name']}}}")  # Keep as placeholder
...
if __name__ == "__main__":
    spec_file = "/home/az/Code/http_file_generator/httpfilegen/openapi.json"  # Input spec file
