...
def test_handle_body_with_example_precedence() -> None:
    rb = RequestBody(
...
def test_handle_body_with_examples_precedence() -> None:
    rb = RequestBody(
        {
            "application/json": Media(
                examples={"a": {"foo": "bar"}}, schema=Schema({"type": "object"})
            )
        }
    )
    out = handle_body("/x", rb)
    ...
    assert body == {"foo": "bar"} or (
        hasattr(body, "value") and getattr(body, "value") == {"foo": "bar"}
    )
