...
def test_example_takes_precedence_over_examples():
    rb = RequestBody({
        "application/json": Media(example={"chosen": True}, examples={"ex": {"chosen": False}}, schema=Schema({"type": "object"}))
    })
...
def test_no_example_no_schema_leads_to_none_key_and_no_header():
    rb = RequestBody({
